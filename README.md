...
- 目标对标：设定科目与高考日期后，首页自动对比最新总分与 2026 广东投档线，含倒计时与达成率（数据在 `admission-data.js`，每年更新）
- 删除保护：删除考试前自动在本机留档，删除后 8 秒内可「撤销」，账号页提供回收站（本机最多 10 条）可随时恢复整场考试
- 市级排名：考试弹窗支持录入市排名与每科市排，趋势图/雷达图口径新增「市排」，记录页显示市排徽章
- 我的选科：账号页敲定选科后，首页趋势科目按钮只保留所选科目（附「＋其余科目」临时展开），历史数据不受影响
- 排名优先：首页「较上次」只做位次对比（名次或位比，年排/班排/市排自动取可用口径）；无排名数据时显示「补填排名」引导，点击直达最近一次考试；副标题同步展示当前年排/市排；有排名数据时趋势图默认「名次」视图
- 图片识别成绩单：考试弹窗支持拍照/截图识别，本地 OCR（Tesseract.js，引擎与中文语言包随项目分发在 `/ocr`，约 9.5MB）自动填入各科分数、年排/班排/市排与总排名，图片不上传、无第三方依赖；识别支持行式与表格型布局，结果请核对后保存
...
入口 `index.html` 依次加载 `compat.js`、`request-budget-v24.js`、`telemetry-feedback.js`、`feedback-statuses.js`、`app.js` 与 `university-theme.js`。`app.js` 由历史版本文件 app-v3 ~ app-v29 按原加载顺序合并而成（纯合并、逻辑不变），文件边界以 `/* ===== 文件名 ===== */` 注释分隔。`university-theme.js` 为梦想大学主题模块，通过 CSS 变量与 DOM 注入实现，不改动 `app.js` 的既有逻辑。反馈表单内容会自动在本机存草稿，提交后清除。`/ocr` 目录为本地 OCR 静态资源（按需加载，不阻塞首屏）。
